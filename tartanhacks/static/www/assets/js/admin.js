...
app.api.promote_user = function(userID, next) {
  if (app.profile.isAdmin) {
    return $.ajax({
      url: "/api/users/" + userID,
      type: 'PUT',
...
app.api.demote_user = function(userID, next) {
  if (app.profile.isAdmin) {
    return $.ajax({
      url: "/api/users/" + userID,
      type: 'PUT',
...
app.api.mentorize_user = function(userID, next) {
  if (app.profile.isAdmin) {
    return $.ajax({
      url: "/api/users/" + userID,
      type: 'PUT',
...
app.api.dementorize_user = function(userID, next) {
  if (app.profile.isAdmin) {
    return $.ajax({
      url: "/api/users/" + userID,
      type: 'PUT',
...
app.router = function() {
  ...
  switch (route) {
    ...
    case 'admin-list':
      return app.routes.adminlist();
    case 'mentor-list':
      return app.routes.mentorlist();
    case 'checked-in':
      return app.routes.checkedinlist();
    default:
...
app.lists.promoteHandler = function(id) {
  app.api.promote_user(id, function() {
    return app.router();
  });
  return false;
};

app.lists.demoteHandler = function(id) {
  app.api.demote_user(id, function() {
    return app.router();
  });
  return false;
};

app.lists.mentorizeHandler = function(id) {
  app.api.mentorize_user(id, function() {
    return app.router();
  });
  return false;
};

app.lists.dementorizeHandler = function(id) {
  app.api.dementorize_user(id, function() {
    return app.router();
  });
  return false;
};

app.templates.checkedinlist = Handlebars.compile($('#checkedin-list-template').html());

app.routes.checkedinlist = function() {
  return app.api.get_users(function(users) {
    users = users.filter(function(elem) {
      return (elem.isCheckedIn != null) && elem.isCheckedIn;
    });
    return $('#content').html(app.templates.checkedinlist({
      users: users
    }));
  });
};
...
app.templates.adminlist = Handlebars.compile($('#admin-list-template').html());

app.routes.adminlist = function() {
  return app.api.get_users(function(users) {
    users = users.filter(function(elem) {
      return (elem.isAdmin != null) && elem.isAdmin;
    });
    return $('#content').html(app.templates.adminlist({
      users: users
    }));
  });
};

app.templates.mentorlist = Handlebars.compile($('#mentor-list-template').html());

app.routes.mentorlist = function() {
  return app.api.get_users(function(users) {
    users = users.filter(function(elem) {
      return (elem.isMentor != null) && elem.isMentor;
    });
    return $('#content').html(app.templates.mentorlist({
      users: users
    }));
  });
};
