...
app.api.make_team = function(next) {
  if (app.profile.loggedIn) {
    return $.post('/api/teams').done(next);
  }
};

app.api.edit_team = function(data, next) {
  if (app.profile.loggedIn) {
    return $.ajax({
      type: 'POST',
      url: '/api/myteam',
...
app.api.get_my_team = function(next) {
  if (app.profile.loggedIn) {
    return $.get('/api/myteam', null).done(function(user) {
      user = JSON.parse(user);
      return next(user);
    }).fail(function() {
      return next(null);
...
app.api.delete_my_team = function(next) {
  if (app.profile.loggedIn) {
    return $.ajax({
      type: 'DELETE',
      url: '/api/myteam'
...
app.api.make_a_team = function(next) {
  if (app.profile.loggedIn) {
    return $.post('/api/teams').done(next).fail(app.err);
  }
};

app.api.get_teams = function(next) {
  if (app.profile.loggedIn) {
    return $.get('/api/teams').done(next).fail(app.err);
  }
};
...
app.team.hasTeam = false;

app.team.formHandler = function(e) {
  var team;
  team = $('#team-form').serialize();
  app.api.edit_team(team, function() {
    return location.href = '/#/team';
  });
  return e.preventDefault();
};

app.team.deleteBtnHandler = function(e) {
  if ($('#team-delete-text').val() === 'delete') {
    return app.api.delete_my_team(app.router);
  }
};

app.templates.team = Handlebars.compile($('#team-template').html());

app.routes.team = function(options) {
  if (options == null) {
    options = {};
  }
  options.loggedIn = app.profile.loggedIn;
  if (!options.loggedIn) {
    $('#content').html(app.templates.team(options));
    return app.google.renderBtn();
  } else {
    return app.api.get_my_team(function(team) {
      options.team = team;
      $('#content').html(app.templates.team(options));
      $('#team-delete').on('click', app.team.deleteBtnHandler);
      $('#create-team-btn').on('click', function() {
        return app.api.make_a_team(app.router());
      });
      if (options.edit) {
        $('#team-form').submit(app.team.formHandler);
        return $('#team-form-submit').on('click', app.team.formHandler);
      }
    });
  }
};
